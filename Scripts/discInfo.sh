...
echo "$(date)"
./discord.sh --webhook-url="$WEBHOOK" --text  "$(date)"
excluded_containers=("$@")
function update_container() {
  ...
  if [[ "$os" == "ubuntu" || "$os" == "debian" ]]; then
    ...
    ./discord.sh --webhook-url="$WEBHOOK" --text  "\n[Info] Updating $container : $name - Boot Disk: ${disk_info_array[0]}% full [${disk_info_array[1]}/${disk_info_array[2]} used, ${disk_info_array[3]} free]"
  else
    echo -e "\n[Info] Updating $container : $name - [No disk info for ${os}]"
  fi
  case "$os" in
    ubuntu | debian | devuan) pct exec "$container" -- bash -c "apt-get update 2>/dev/null | grep 'packages.*upgraded'; apt list --upgradable" ;;
  esac
}
...
./discord.sh --webhook-url="$WEBHOOK" --text  "Finished, All Containers Updated. \n"
